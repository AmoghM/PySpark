...
def find_friends_by_age():
    ...
    social_media_rdd = social_media_dataset.map(read_data)

    key_value_rdd = social_media_rdd.mapValues(lambda x: (x, 1)).reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
    avg_friends = key_value_rdd.mapValues(lambda x: x[0] / x[1])
